...
import { getToken, onAuthExpired, BASE } from '../services/client'
...
export function LecturerProvider({ children }) {
  ...
  // Listen for auth expiry from client.js
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`${wsUrl}/ws/sessions/${openSession.id}?token=${token}`)
